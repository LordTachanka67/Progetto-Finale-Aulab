{{-- <div class="card mb-5" >
  ...
  <div class="card mb-5">
    ...
    @else
    <div>
        <span class="badge position-absolute">{{__('ui.categoria')}}: {{__('ui.' . $article->category->name)}}</span>
    </div>    
    @endif
    ...
    <a href="{{route('articles.show', compact('article'))}}" class="card-button btn-quar text-decoration-none text-center">{{__('ui.vaiArticolo')}}</a>
  </div>